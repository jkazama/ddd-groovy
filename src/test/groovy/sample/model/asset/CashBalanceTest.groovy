...
//low: 簡易な正常系検証のみ
class CashBalanceTest extends UnitTestSupport {
	...
	public void getOrNew() {
		...
		// 存在している残高の検証
		def cbNormal = CashBalance.getOrNew(rep, "test1", "JPY")
		...
		// 基準日に存在していない残高の繰越検証
		def cbRoll = CashBalance.getOrNew(rep, "test2", "JPY")
		...
		// 残高を保有しない口座の生成検証
		def cbNew = CashBalance.getOrNew(rep, "test3", "JPY")
